<?php
    ...
    if (isset($_GET['person'])) {
        ...
        foreach ($data as $i=>$val) {
            ...
            $query = mysqli_query($db, $SQL) or die($SQL . "|Couldn't execute query." . mysqli_error($db));
            $tag =implode(',',mysqli_fetch_all($query, MYSQLI_ASSOC));
            $data[$i]['tag']=$tag;
            ...
            $query = mysqli_query($db, $SQL) or die($SQL . "|Couldn't execute query." . mysqli_error($db));
            $tag =implode(',',mysqli_fetch_all($query));
            $data[$i]['sci_theme']=$tag;
            /*Файлы*/
            $SQL="SELECT file.Name FROM file,(select * from file_person where idPerson={$val['id']}) as file_person
